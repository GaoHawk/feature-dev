<template>
  <div>
    <div class="layout-header" v-for="(item,key) in searchArrBtn" style="width:172px;margin-right:5px;">
      ...
      <DatePicker type="datetime"  placement="bottom-end"  v-model="searchArrBtn[key].value" format="yyyy-MM-dd HH:mm:ss" :placeholder="'请输入' + item.label"  v-if=" key === 'startTime' " :style="[item.label.length == 4?{width:'70%'}:item.label.length==3?{width:'77%'}:{width:'84%'}]">
          ...
          return h("div", [
            ...
            h(
              "Button",
              {
                ...
                on: {
                  click: () => {
                    vm.$refs.list.travel(params.row.id);
                  }
                }
              },
              "设计行程"
