<template>
  <div>
    <div class="layout-header" v-for="(item,key) in searchArrBtn" style="width:172px;margin-right:5px;">
      ...
      <Select filterable v-if="key === 'continent' " :placeholder="'请选择' + item.label" v-model="searchArrBtn[key].value" :style="[item.label.length == 4?{width:'70%'}:item.label.length==3?{width:'77%'}:{width:'84%'}]">
    ...
    <citym ref="list" :columns = "tableColumns"> </citym>
  </div>
